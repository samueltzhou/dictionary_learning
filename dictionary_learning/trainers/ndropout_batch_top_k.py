...
class NDropoutBatchTopKTrainer(SAETrainer):
    """Nested-Dropout variant of BatchTopK SAE.

    Instead of the coarse Matryoshka grouping, we construct *all* prefixes of the top-k
    latent activations (top-1, top-2, …, top-k) and average their reconstruction loss.
    This is equivalent to Matryoshka where every group has size 1.
    """
    ...
    def __init__(
        ...
        wandb_name: str = "NDropoutBatchTopKSAE",
    ...
    def loss(self, x, step=None, logging=False):
        ...
        # Start with bias-only reconstruction.
        x_reconstruct = t.zeros_like(x) + self.ae.b_dec
        l2_losses = []

        for i in range(self.k):
            # Prepare sparse activation tensor containing just the i-th strongest latent.
            acts_slice = t.zeros_like(f)
            acts_slice.scatter_(1, idx_BK[:, i : i + 1], vals_BK[:, i : i + 1])

            # Incremental reconstruction.
            x_reconstruct = x_reconstruct + acts_slice @ self.ae.W_dec
    ...
    def config(self):
        return {
            "trainer_class": "NDropoutBatchTopKTrainer",
